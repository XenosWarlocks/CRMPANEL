...
export const GitHubBanner = () => {
    useEffect(() => {
        ...
        CSSRules.forEach((rule) => styleTag.sheet?.insertRule(rule, styleTag.sheet.cssRules.length),)
    }, [])
...
const Text = () => {
    return (
        <a
            className="gh-link"
            href="https://github.com/XenosWarlocks/CRMPANEL"
            target="_blank"
            rel="noreferrer"
            style={{
                position: "absolute",
...
const GlowBig = ({ style, ...props }: SVGProps<SVGSVGElement>) => (
    ...
    >
        <circle cx={60} cy={24} r={60} fill={`url(#${props.id}-a)`} fillOpacity={0.5} />
        <defs>
            <radialGradient
                id={`${props.id}-a`}
                cx={0}
